...
XF_CASE_ROOT     ?= $(CUR_DIR)


#MK_COMMON_DIR := mk

# Below should point to library repo
XF_PROJ_ROOT    ?= $(abspath $(XF_CASE_ROOT)/../../..)
...
help::
	...
	@echo ""	
	@echo "  make host xclbin TARGET=<sw_emu|hw_emu|hw> DEVICE=<FPGA platform> BOARD=Zynq ARCH=<aarch32 | aarch64>"
...
KSRC_DIR 		:= $(XF_PROJ_ROOT)/L3/examples/stereopipeline
...
XCLBIN_NAME 	:= krnl_stereopipeline
KER_NAME    	:= stereopipeline_accel
KERNELS += $(KER_NAME):xf_stereo_pipeline_accel.cpp

VPP_CFLAGS  	+= -I$(XF_PROJ_ROOT)/include -I. -I$(KSRC_DIR)/build -I$(XF_PROJ_ROOT)/L1/include
...
SRC_DIR 		:= $(XF_PROJ_ROOT)/L3/examples/stereopipeline

EXE_NAME  		:= stereopipeline
ifeq ($(TARGET), hw_emu)
HOST_ARGS 		= $(XF_PROJ_ROOT)/data/left_emu.png $(XF_PROJ_ROOT)/data/right_emu.png
else
HOST_ARGS 		= $(XF_PROJ_ROOT)/data/left.png $(XF_PROJ_ROOT)/data/right.png
endif #$(XCLBIN_FILE)
...
# Search paths:
CXXFLAGS 		+= -I$(XF_PROJ_ROOT)/include -I. -I$(KSRC_DIR)/build -I$(XF_PROJ_ROOT)/L1/include -I$(XF_PROJ_ROOT)/ext/xcl2
# Options
CXXFLAGS 		+= -g
...
RUN_ENV +=	export LD_LIBRARY_PATH=$(LD_LIBRARY_PATH):$(OPENCV_LIB);
    ...
    opencv_LDFLAGS	+= -lopencv_core -lopencv_imgproc -lopencv_highgui -lopencv_calib3d -lopencv_features2d -lopencv_flann -lopencv_imgcodecs
...
CXXFLAGS += -idirafter $(XILINX_VITIS)/include
...
ifeq ($(TARGET), $(filter $(TARGET),sw_emu hw_emu))
       EMU_CMD = $(PERL) $(XF_PROJ_ROOT)/ext/make_utility/embedded_run_emulation.pl "./$(PKG_OUT_DIR)/launch_$(TARGET).sh -no-reboot" "./run_script.sh" "" "12"
else ifeq ($(TARGET), hw)
       RUN_ENV += 
...
ifeq ($(TARGET), $(filter $(TARGET),sw_emu hw_emu))
	...
	v++ -t $(TARGET) --platform $(DEVICE) -p $(XCLBIN_FILE) --package.out_dir $(PKG_OUT_DIR) --package.rootfs $(SYSROOT)/../../rootfs.ext4 --package.sd_file $(ROOTFS) --package.sd_file run_script.sh --package.sd_file $(EXE_FILE) $(SD_FILE_LIST) --package.sd_file emconfig.json
...
CXXFLAGS += -std=c++14 -fPIC -I$(OPENCV_INCLUDE) \
	-I$(SRC_DIR) -I$(XILINX_XRT)/include -I$(XILINX_VITIS)/include \
	-Wall -Wno-unknown-pragmas -Wno-unused-label -pthread
CFLAGS +=
LDFLAGS += -pthread -L$(XILINX_XRT)/lib -lxilinxopencl
